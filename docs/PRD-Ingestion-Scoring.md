...
The story's "preserved through every re-run" clause depends on two further requirements:
...
| Pinned context | The engine scores, scoring-plan version, and `state_requirements_tag` at decision time, so a later run can detect that the engine has moved out from under a standing decision |

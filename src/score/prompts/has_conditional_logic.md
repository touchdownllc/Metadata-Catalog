{# Phase A prompt template for `has_conditional_logic`. Rendered with
   str.format_map (see prompts.render_prompt in extract.py). The rules +
   examples block is the **policy-side mitigation** for POC-2's inline
   regex in `decision_tree.py:78`. Policy lives in one of two places —
   the prompt, or (Phase C, only if measurably needed) a YAML policy
   file. Never inline in Python rules.
